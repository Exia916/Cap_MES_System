// pages/api/admin/laser-production-all.ts
...
const LASER_SHEET_NAME = "Laser Production";
const USERS_SHEET_NAME = "Emb Users";

type LaserRow = {
  rowNumber: number;   // actual sheet row (2, 3, 4, ...)
  values: string[];    // columns A..H
};

type SuccessResponse = {
  headers: string[];   // column headers A..H
  entries: LaserRow[];
};
...
export default async function handler(
  ...
  try {
    ...
    // ---- Headers A1:H1 ----
    const headerRange = `${LASER_SHEET_NAME}!A1:H1`;
    const headerResp = await sheets.spreadsheets.values.get({
      spreadsheetId: SHEET_ID,
      range: headerRange,
    });
    ...
    // ---- Data rows A2:H ----
    const dataRange = `${LASER_SHEET_NAME}!A2:H`;
    const dataResp = await sheets.spreadsheets.values.get({
      spreadsheetId: SHEET_ID,
      range: dataRange,
    });
    ...
    const entries: LaserRow[] = rows.map((row, index) => ({
    ...
    console.error("Error listing Laser Production (admin):", err);
    return res
      .status(500)
      .json({ error: "Failed to load Laser Production entries" });
  }
}
