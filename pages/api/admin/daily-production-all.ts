// pages/api/admin/daily-production-all.ts
...
const DAILY_SHEET_NAME = "Daily Production";
const USERS_SHEET_NAME = "Emb Users";

type DailyRow = {
  rowNumber: number;   // actual sheet row (2, 3, 4, ...)
  values: string[];    // columns A..S
};

type SuccessResponse = {
  headers: string[];   // column headers A..S
  entries: DailyRow[];
};
...
// Find a column index by header name (case-insensitive, supports "Role"/"Roles")
...
async function getUserRoles(
  ...
  // 2) Read data rows A2:Z  ✅ THIS IS WHERE THE BUG WAS
  const dataRange = `${USERS_SHEET_NAME}!A2:Z`;
  const dataResp = await sheets.spreadsheets.values.get({
    spreadsheetId: SHEET_ID,
    range: dataRange, // <— use dataRange, not "range"
  });
  ...
  for (const row of rows) {
    ...
    // STRONGER MATCHING: require exact match on username OR displayName
...
export default async function handler(
  ...
  try {
    ...
    // ---- Headers A1:S1 ----
    const headerRange = `${DAILY_SHEET_NAME}!A1:U1`;
    const headerResp = await sheets.spreadsheets.values.get({
      spreadsheetId: SHEET_ID,
      range: headerRange,
    });
    ...
    // ---- Data rows A2:S ----
    const dataRange = `${DAILY_SHEET_NAME}!A2:U`;
    const dataResp = await sheets.spreadsheets.values.get({
      spreadsheetId: SHEET_ID,
      range: dataRange,
    });
    ...
    const entries: DailyRow[] = rows.map((row, index) => ({
    ...
    console.error("Error listing daily production (admin):", err);
    return res
      .status(500)
      .json({ error: "Failed to load daily production entries" });
  }
}
