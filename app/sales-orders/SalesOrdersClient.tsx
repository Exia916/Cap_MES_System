...
export default function SalesOrdersClient({ initialSo = "" }: Props) {
  const router = useRouter();

  const normalizedInitialSo = initialSo.trim();

  const [salesOrderInput, setSalesOrderInput] = useState(normalizedInitialSo);
  ...
  async function runLookup(so: string) {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error((body as { error?: string })?.error || "Failed to load Sales Order.");
      }
      ...
      const message =
        e instanceof Error ? e.message : "Failed to load Sales Order.";
      setResult(null);
      setError(message);
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    if (!normalizedInitialSo) return;
    void runLookup(normalizedInitialSo);
    // intentionally run once for initial page param
              ...
              <div className="card">
                ...
                <div className="so-kv-list">
                  ...
                  {line(
                    ...
                    shipTo?.AdditonalDeliveryInstruction ||
                      shipTo?.AdditionalDeliveryInstruction
                  )}
                </div>
              </div>
            </div>
