...
export default function KnitQcAllTable({
  defaultStart,
  defaultEnd,
}: {
  defaultStart: string;
  ...
  async function load(qs: string) {
    ...
    try {
      const res = await fetch(`/api/admin/knit-qc-all?${qs}`, {
        cache: "no-store",
      });
  ...
  function exportCsv() {
    ...
    p.set("format", "csv");
    window.location.href = `/api/admin/knit-qc-all?${p.toString()}`;
  }
  ...
  return (
    <div style={{ border: "1px solid #ddd", borderRadius: 14, padding: 12, width: "100%" }}>
      ...
      <div style={{ display: "flex", gap: 18, marginBottom: 10, color: "#111827", flexWrap: "wrap" }}>
        <div style={{ fontWeight: 800 }}>Total Inspected: {fmtInt(totals?.total_inspected ?? 0)}</div>
        <div style={{ fontWeight: 800 }}>Total Rejected: {fmtInt(totals?.total_rejected ?? 0)}</div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13, minWidth: 1650 }}>
          <thead>
            ...
            <tr>
              ...
              <th style={{ padding: 6, borderBottom: "1px solid #ddd", background: "#fff" }}>
                <input value={salesOrder} onChange={(e) => setSalesOrder(e.target.value)} placeholder="SO" style={{ ...input, width: 130 }} />
              </th>
              ...
              <th style={{ padding: 6, borderBottom: "1px solid #ddd", background: "#fff" }}>
                <input disabled placeholder="(inspected)" style={{ ...input, width: 100, opacity: 0.55 }} />
              </th>
