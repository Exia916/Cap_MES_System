...
type Resp = { entries: LaserEntry[] } | { error: string };
  ...
  try {
    const sql = `
      SELECT
        ...
        leather_style_color AS "leatherStyleColor",
        pieces_cut AS "piecesCut",
        notes
      FROM laser_entries
    ...
    const result = await db.query<LaserEntry>(sql, [auth.employeeNumber, entryDate]);
