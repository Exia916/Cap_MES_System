...
import {
  listKnitSubmissionSummariesRange,
  type KnitSubmissionSummaryRow,
...
type Resp =
  | {
      submissions: KnitSubmissionSummaryRow[];
      totalCount: number;
    }
  | { error: string };
  ...
  try {
    ...
    if (!roleOk(auth.role, VIEW_ROLES)) {
      await logWarn({
        ...
        eventType: "KNIT_PRODUCTION_LIST_FORBIDDEN",
        message: "User attempted to view knit production list without permission",
      });
    ...
    const includeVoided =
      isPowerUser && sp.get("includeVoided") === "true";

    const onlyVoided =
      isPowerUser && sp.get("onlyVoided") === "true";
    ...
    const result = await listKnitSubmissionSummariesRange({
      entryDateFrom,
      entryDateTo,
      employeeNumber,
      name: trimmed(sp.get("name")),
      salesOrderStartsWith: trimmed(sp.get("salesOrder")),
      itemStyle: trimmed(sp.get("itemStyle")),
      logo: trimmed(sp.get("logo")),
      notes: trimmed(sp.get("notes")),
      knitArea: trimmed(sp.get("knitArea")),
    ...
    await logError({
      ...
      category: "API",
      module: "KNIT_PRODUCTION",
      eventType: "KNIT_PRODUCTION_LIST_ERROR",
      message: "Failed to load knit production submissions",
    ...
    console.error("knit-production-submission-list GET error:", err);
