...
export async function PUT(req: Request, ctx: { params: Promise<{ key: string; id: string }> }) {
  ...
  const setUpdatedAt = cfg.selectCols.includes("updated_at") ? `, updated_at = NOW()` : "";
...
export async function DELETE(_req: Request, ctx: { params: Promise<{ key: string; id: string }> }) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: process.env.NODE_ENV === "production" ? "Failed to delete row" : err?.message || "Failed to delete row" },
      { status: 500 }
    );
  }
}