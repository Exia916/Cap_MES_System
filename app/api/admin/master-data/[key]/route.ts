// app/api/admin/master-data/[key]/route.ts
...
function getFromClause(key: MasterKey) {
  if (key === "cmms_assets") {
    return `
      FROM cmms.assets a
      JOIN cmms.departments d ON d.id = a.department_id
    `;
  }

  return `FROM ${MASTER_DATA[key].table}`;
}
...
export async function GET(_req: Request, ctx: { params: Promise<{ key: string }> }) {
  ...
  try {
    ...
    return NextResponse.json({
      rows: res.rows,
      supportsInactive: cfg.supportsInactive,
    ...
    return NextResponse.json(
      {
        error:
          process.env.NODE_ENV === "production"
            ? "Failed to load rows"
            : err?.message || "Failed to load rows",
      },
      { status: 500 }
    );
  }
