...
import { listRecutReasons } from "@/lib/repositories/recutRepo";

export const runtime = "nodejs";

type Resp =
  | {
      rows: Awaited<ReturnType<typeof listRecutReasons>>;
    }
  | { error: string };
...
export async function GET(req: NextRequest) {
  try {
    ...
    const rows = await listRecutReasons();
    return NextResponse.json<Resp>({ rows }, { status: 200 });
  } catch (err) {
    console.error("recut reasons lookup GET error:", err);
