...
import { listRecutRequestsForWarehousePaged, type SortDir } from "@/lib/repositories/recutRepo";
  ...
  try {
    ...
    if (!roleOk((auth as any).role)) {
      await logWarn({
        ...
        category: "API",
        module: "RECUT",
        eventType: "RECUT_WAREHOUSE_LIST_FORBIDDEN",
        message: "User attempted to load recut warehouse list without permission",
      });
    ...
    const result = await listRecutRequestsForWarehousePaged({
      page,
      pageSize,
      q,
      recutId,
    ...
    await logError({
      ...
      category: "API",
      module: "RECUT",
      eventType: "RECUT_WAREHOUSE_LIST_ERROR",
      message: "Failed to load recut warehouse list",
      error: err,
      details: {
        code: err?.code ?? null,
    ...
    console.error("recuts warehouse list GET error:", err);
